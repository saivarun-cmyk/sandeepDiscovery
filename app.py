...
def render_market(market):
    ...
    scan_key = market + '_scan'
    error_key = market + '_scan_error'

    latest = latest_date(market)
    with st.container():
        st.subheader(config['label'])
        st.caption(f"Prices in {config['currency']}. Same-day data after {config['ready_hour']}:00 {config['timezone']}.")
        if market == 'US':
            st.info('45 equities + 2 COMEX futures proxies. GC1! → GC=F, SI1! → SI=F; futures rolls can differ from TradingView. BRK.A/B use BRK-A/B on Yahoo.')
        as_of = st.date_input('Completed trading session', value=latest, max_value=latest, key=market+'_date')
        st.caption('Daily closing bars. For exchange holidays, select the previous trading session.')
        st.metric('Instruments in universe', len(active))
        st.caption('Every Run Scan click requests Yahoo data again for every enabled stock. Prices may be delayed by Yahoo.')
        clicked = st.button('Run Scan', type='primary', width="stretch", key=market+'_run')
    ...
    if clicked:
        ...
        try:
            st.session_state[scan_key] = run_scan(as_of, lambda n, total, symbol: bar.progress(n / total, text=f'{n}/{total} stocks fetched · {symbol}'), market=market)
    ...
    if st.session_state.get(error_key):
        st.error('Scan failed: ' + st.session_state[error_key])
    ...
    tabs = st.tabs([f'Bullish Top {top_n}', f'Bearish Top {top_n}', 'All stocks', 'Data issues',
    ...
    if scan:
        ...
        def show(rows, fields, name):
            ...
            st.download_button('Download CSV', frame.to_csv(index=False).encode('utf-8'),
                               file_name=f'{market}_{name}_{scan["as_of"]}.csv', mime='text/csv', key=market+'_download_'+name)
        with tabs[0]:
            show(scan['bullish'], FIELDS + ['exchange','asset_type','yahoo_symbol','currency'], f'bullish_top{top_n}')
        with tabs[1]:
            show(scan['bearish'], FIELDS + ['exchange','asset_type','yahoo_symbol','currency'], f'bearish_top{top_n}')
        with tabs[2]:
            search = st.text_input('Search symbol', key=market+'_search').strip().upper()
            ...
            for row, item in zip(all_rows, scan['universe']):
                row.update(exchange=item.get('exchange', 'NSE'), asset_type=item.get('asset_type', 'Stock'), yahoo_symbol=item['yahoo_symbol'], currency=config['currency'])
            ...
            show([r for r in all_rows if search in r['symbol']], FIELDS + INDICATOR_FIELDS + ['exchange', 'asset_type', 'yahoo_symbol', 'currency', 'issue', 'indicator_issue'], 'all_stocks')
        with tabs[3]:
            if not scan['errors']:
        ...
        with tabs[4]:
            st.caption('Selected market universe. Ranked by absolute distance to EMA13, closest first.')
        ...
        with tabs[5]:
            ...
            only_pass = st.checkbox('Show only Darvas matches', value=True, key=market+'_darvas')
        ...
        with tabs[6]:
            ...
            side = st.selectbox('Master Trend direction', ['All', 'BULLISH', 'BEARISH'], key=market+'_trend')
        ...
        with tabs[7]:
            ...
            st.caption(f'Discovery Top {top_n} lists with your setup indicators. These are review candidates, not trade approvals.')
...
india, usa = st.tabs(['Indian stocks', 'US stocks'])
with india:
    render_market('IN')
with usa:
    render_market('US')
